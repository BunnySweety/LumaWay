...
- `scripts/phase2-field-preflight.sh --require-camera no --camera-fps 120`: helper verifies Phase 2 helper/harness presence without requiring a local camera when a phone/non-V4L2 capture FPS is declared; `--require-camera no` without `--camera-fps`, `--camera-fps 60`, and the missing-camera path with `--video-device /tmp/lumaway-missing-camera` all exit non-zero.
