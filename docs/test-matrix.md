...
- StatusNotifier/AppIndicator Start/Stop when the desktop exposes a tray: tray menu shows `Show LumaWay`, Start/Stop according to sync state, and Quit asks for confirmation while sync is active.
- GNOME/KDE tray watcher late/absent: tray registration must not block startup; window Start/Stop remains the control surface.
...
- `/tmp/lumaway-codex-target/debug/lumaway backend-probe --frames 5 --sample-width 120 --sample-height 68 --fps 25` on commit `593bd0a`: real GNOME Wayland Portal probe selected a 2560x1440 stream, confirmed CPU usable (`frames=5/5`, `max_rgb=188`, `avg_luma=149.5`) while GL returned black (`max_rgb=0`, `avg_luma=0.0`); recommendation was `backend=cpu`.
...
- `docs/phase2-comparison-harness.md` + `docs/fixtures/phase2-patterns.html`: Phase 2.4 harness defines fixed full-screen patterns, `sample-debug` / `capture-quality` commands, the release latency gate of every accepted transition at or below 300 ms, and a field-result template that records preflight, recording FPS, frame pairs, and combined `phase2_field_evidence` output.
- `docs/phase2-validation-2026-05-16.md`: Phase 2 audit maps every 2.1-2.6 deliverable to concrete code/docs/test evidence, records live TV diagnostics, and explicitly leaves only the external visible-latency video, no-silent-black observation, and new-user timing proof open.
- `scripts/phase2-field-preflight.sh --require-camera no --camera-fps 120`: helper verifies Phase 2 helper/harness presence without requiring a local camera when a phone/non-V4L2 capture FPS is declared; `--require-camera no` without `--camera-fps`, `--camera-fps 60`, `--require-camera no` combined with `--video-device`, and the missing-camera path with `--video-device /tmp/lumaway-missing-camera` all exit non-zero.
- `scripts/phase2-latency-summary.sh --fps 120 100:124 220:247 340:369 460:490 580:613`: helper converts video frame pairs into latency milliseconds and passes the 5-transition 300 ms gate; failure cases with one 400 ms transition, fewer than 5 transitions, or `--min-transitions 0` exit non-zero.
- `scripts/phase2-first-run-summary.sh --elapsed-seconds 420 --calibrate-used no`: helper passes the 10-minute no-calibration first-run gate; `--elapsed-seconds 601` or `--calibrate-used yes` exits non-zero.
- `scripts/phase2-field-evidence.sh --fps 120 --elapsed-seconds 420 --preflight-output <phase2-preflight.txt> --calibrate-used no --silent-black no 100:124 220:247 340:369 460:490 580:613`: helper prints one combined evidence block and exits non-zero if the saved preflight gate, video-FPS gate, visible-latency gate, first-run gate, or no-silent-black gate fails; command-error cases cover missing or ambiguous preflight output, simultaneous `--preflight-output` / `--preflight-pass`, and invalid `--min-transitions 0`.
- `/tmp/lumaway-codex-target/debug/lumaway sample-debug --portal --sync-mode video --preset video-wayland --area TV --frames 1 --sampling region --color-profile vivid` on commit `593bd0a`: real GNOME Wayland Portal capture used CPU, sampled the two TV channels with `sampling=Region`, and produced non-black per-channel output.
- `/tmp/lumaway-codex-target/debug/lumaway capture-quality --portal --sync-mode video --preset video-wayland --area TV --frames 10 --sampling region --color-profile vivid` on commit `593bd0a`: real TV-area capture used CPU with `dark_frames=0`; remaining recommendation was content-related (`low_temporal_variation`, `low_saturation`) rather than black capture.
- `/tmp/lumaway-codex-target/debug/lumaway sync --sync-mode video --preset video-wayland --area TV --duration-ms 3000 --sampling region --color-profile vivid --capture-backend auto` on commit `593bd0a`: real TV sync started without `calibrate-capture`, detected black GL output, fell back to CPU, completed Hue DTLS handshake, sent 75 frames, and deactivated Entertainment after the requested duration. `sync_stats` reported `capture_max_ms=20.069` and `send_max_ms=4.886`, so the internal 25 Hz latency guard is ~105 ms (`20.069 + 4.886 + 80`) before Hue/light emission; the visible ≤300 ms gate still requires external video evidence.
