...
- `scripts/phase2-field-preflight.sh --require-camera no`: helper verifies Phase 2 helper/harness presence without requiring a camera; the missing-camera path exits non-zero when `--video-device /tmp/lumaway-missing-camera` is used.
