...
- `scripts/phase2-field-preflight.sh --require-camera no`: helper verifies Phase 2 helper/harness presence without requiring a camera; the declared-FPS path passes with `--camera-fps 120` and exits non-zero with `--camera-fps 60`; the missing-camera path exits non-zero when `--video-device /tmp/lumaway-missing-camera` is used.
