...
| 2.3 | Livré côté code : si la sync échoue sur une capture trop sombre/noire, la carte d’assistant réapparaît avec l’action **Probe backend** ; elle lance `lumaway backend-probe` et affiche un résumé CPU/GL dans le journal. |
| 2.4 | Livré : harness documenté dans [`phase2-comparison-harness.md`](phase2-comparison-harness.md) + fixture [`fixtures/phase2-patterns.html`](fixtures/phase2-patterns.html) ; motifs fixes, commandes `backend-probe` / `capture-quality` / `sample-debug`, seuil de latence **≤ 300 ms** par transition acceptée et modèle de résultats. |
| 2.5 | Livré : les diagnostics s'appuient sur `lumaway sample-debug` et `capture-quality`, réutilisables depuis le flux GUI et les commandes de validation. |
| 2.6 | Livré côté code : `position.y` reste l'axe vertical prioritaire ; si le pont expose une variation `position.z` mais pas de variation `y`, `z` sert de projection verticale de repli pour le sampling. |
...
1. Produire les preuves terrain restantes Phase 2 via [`phase2-comparison-harness.md`](phase2-comparison-harness.md) : lancer `scripts/phase2-field-preflight.sh | tee phase2-preflight.txt` sur la machine cible, ou `scripts/phase2-field-preflight.sh --require-camera no --camera-fps 120 | tee phase2-preflight.txt` pour une caméra téléphone/non-V4L2, filmer le harness couleur avec écran + lampes à 120 fps ou plus, mesurer au moins 5 transitions acceptées toutes à 300 ms ou moins, et confirmer qu'aucun motif non noir ne reste noir silencieusement.
2. Chronométrer un parcours nouvel utilisateur jusqu'au premier sync écran satisfaisant, sans `calibrate-capture`, avec objectif 10 min ou moins, puis valider l'ensemble avec `scripts/phase2-field-evidence.sh --preflight-output phase2-preflight.txt --silent-black no ...`.
...
| 2 | Livré côté code/docs | 2.1 profils capture avec `LUMAWAY_SAMPLE_CROP_*`. 2.2 anti-noir doux du profil Vidéo / `vivid`. 2.3 action `Probe backend` proposée après capture noire. 2.4 harness + fixture patterns. 2.5 diagnostics `sample-debug` / `capture-quality`. 2.6 projection `position.z` en repli vertical quand `y` est plat. Validation release restante : filmer le harness sur matériel réel pour confirmer la latence visible ≤ 300 ms, confirmer l'absence de session noire silencieuse et chronométrer un parcours nouvel utilisateur ≤ 10 min. |
| 3 | À faire | |
| 4 | À faire | |
...
| 2026-05-16 | Phase 2 critères terrain restants | Harness Phase 2 enrichi avec garde-fou interne `sync_stats`, champs de preuve vidéo ≤ 300 ms, observation absence de session noire silencieuse et chronométrage nouvel utilisateur ≤ 10 min ; ces preuves restent manuelles car elles exigent caméra / observateur |
