...
The GUI detects the controller automatically at startup when no bridge address is configured. It automatically fetches configured zones when `LUMAWAY_BRIDGE` and `LUMAWAY_APP_KEY` are already available, preserving the saved zone if it is present in the returned list. The main window shows a `First setup` card until bridge, pairing keys, zone, and zone switch are ready; its actions run discovery, pairing, and a short red `test-color` validation on the selected zone before starting screen sync. Manual discovery / zone refresh actions remain available in Settings.
...
When the desktop exposes a StatusNotifier/AppIndicator tray, LumaWay publishes an opportunistic tray menu with `Show LumaWay`, Start/Stop, and Quit. Start/Stop mirrors the main window state. Quit exits immediately when idle; during active sync it presents the main window and asks for confirmation before stopping sync and quitting.
On desktops without a StatusNotifier/AppIndicator tray, the Start/Stop window remains the fallback control surface. Closing the window while sync is active hides it and keeps sync running; launching LumaWay again presents the existing window so Stop remains available. Classified errors also send a minimal desktop notification when the window is hidden or inactive, if the session notification service accepts GApplication notifications.
