//! `sample-debug` command: per-channel capture trace with smoothing.

use anyhow::{bail, Result};
use lumaway_core::{
    CaptureProfile, ColorSmoother, ColorSmoothingConfig, GStreamerTestCapture, PortalScreenCast,
};
...
use crate::{
    ...
    run_sample_debug_loop, validate_brightness, validate_fps, CaptureBackend, ColorProfile,
...
pub async fn run_sample_debug(
    ...
    let profile = CaptureProfile::new(sample_width, sample_height, pipewire_fps)?;
    let (radius_x, radius_y) = profile.point_sample_radius();
    let channel_samples =
    ...
    capture.start()?;
    let result = run_sample_debug_loop(
        &capture,
        &entertainment_area,
        &channel_samples,
        &points,
